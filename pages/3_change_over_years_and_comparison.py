...
def plot_year_comparison_with_arrows(y1_values, y2_values, label1, label2, metrics):
    ...
    fig.add_trace(go.Bar(
        ...
        text=[f"{label1}<br>{selected_year}<br>{v:.3f}" if not pd.isna(v) else "No Data" for v in vals1],
        textposition="inside",
    ...
    fig.add_trace(go.Bar(
        ...
        text=[f"{label2}<br>{selected_year}<br>{v:.3f}" if not pd.isna(v) else "No Data" for v in vals2],
        textposition="inside",
        insidetextanchor="middle",
        cliponaxis=False,
